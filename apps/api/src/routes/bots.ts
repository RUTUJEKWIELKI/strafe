...
const BotInstallResponseSchema = Type.Object({
...
const botRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: CreateBotBody }>(
    ...
    async (request, reply) =>
      reply
        .code(201)
        .send(await app.botService.create(request.auth.userId, request.body)),
  )
  ...
  app.delete<{ Params: { botId: string } }>(
    ...
    async (request) => ({
      revoked: await app.botService.revoke(
        request.auth.userId,
        request.params.botId,
      ),
    }),
  ...
  app.post<{ Params: { botId: string; serverId: string } }>(
