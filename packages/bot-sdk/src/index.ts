...
export class StrafeBot extends EventEmitter {
  ...
  async connect() {
    ...
    const wsUrl =
      (this.options.baseUrl ?? 'https://strafe.app').replace('http', 'ws') +
      '/api/gateway'
  ...
  async sendRawMessage(
    channelId: string,
    body: paths['/api/channels/{channelId}/messages']['post']['requestBody']['content']['application/json'],
  ) {
  ...
  async getMessages(channelId: string, limit: number = 50) {
    ...
    if (error)
      throw new Error(`Failed to fetch messages: ${JSON.stringify(error)}`)
    return data
  }
}
