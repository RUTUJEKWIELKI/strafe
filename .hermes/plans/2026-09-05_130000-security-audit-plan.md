...
export function renderSafeMarkdown(rawMarkdown: string): string {
  ...
  return DOMPurify.sanitize(rawHtml, {
    ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'a', 'p', 'br', 'ul', 'ol', 'li', 'code', 'pre', 'blockquote', 'del'],
...
if (app.config.NODE_ENV === 'production') {
  ...
  if (!outcome.success) {
    throw new AppError({ code: 'CAPTCHA_FAILED', message: 'Bot verification failed', statusCode: 400 })
  }
}
```
**Verification:**
...
export function decryptPII(cipherText: string, key: Buffer): string {
  ...
  if (!iv64 || !authTag64 || !encrypted) throw new Error('Invalid cipher format')
