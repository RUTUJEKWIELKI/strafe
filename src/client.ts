...
import {
  ...
  type ServerTransferOwnershipResponse,
  type ServerListCurrentUserResponse,
  type ServerUpdateParams,
  type ServerTransferOwnershipParams,
} from './resources/servers';
import {
  Channels,
  type ChannelUpdateResponse,
  type ChannelDeleteResponse,
  type ChannelReorderServerResponse,
  type ChannelListServerResponse,
  type ChannelCreateResponse,
  type ChannelUpdateParams,
  type ChannelReorderServerParams,
  type ChannelCreateParams,
} from './resources/channels';
import {
  Permissions,
  type PermissionListChannelOverwritesResponse,
  type PermissionUpsertChannelOverwriteResponse,
  type PermissionDeleteChannelOverwriteResponse,
  type PermissionUpsertChannelOverwriteParams,
  type PermissionDeleteChannelOverwriteParams,
} from './resources/permissions';
import {
  Roles,
  type RoleListServerResponse,
  type RoleCreateResponse,
  type RoleUpdateServerResponse,
  type RoleDeleteServerResponse,
  type RoleReorderServerResponse,
  type RoleCreateParams,
  type RoleUpdateServerParams,
  type RoleDeleteServerParams,
  type RoleReorderServerParams,
} from './resources/roles';
import { Audit, type AuditListServerLogResponse, type AuditListServerLogParams } from './resources/audit';
import {
  DirectMessages,
  type DirectMessageListResponse,
  type DirectMessageCreateResponse,
  type DirectMessageCreateParams,
} from './resources/direct-messages';
import {
  Files,
  type FileInitiateUploadResponse,
  type FilePresignUploadPartResponse,
  type FileCompleteUploadResponse,
  type FileAbortUploadResponse,
  type FileRetrieveResponse,
  type FileDownloadResponse,
  type FileInitiateUploadParams,
  type FilePresignUploadPartParams,
  type FileCompleteUploadParams,
  type FileDownloadParams,
} from './resources/files';
import {
  Members,
  type MemberListServerResponse,
  type MemberLeaveServerResponse,
  type MemberReplaceRolesResponse,
  type MemberListServerParams,
  type MemberReplaceRolesParams,
} from './resources/members';
import {
  Moderation,
  type ModerationKickServerMemberResponse,
  type ModerationClearMemberTimeoutResponse,
  type ModerationTimeoutMemberResponse,
  type ModerationUnbanServerMemberResponse,
  type ModerationBanMemberResponse,
  type ModerationKickServerMemberParams,
  type ModerationClearMemberTimeoutParams,
  type ModerationTimeoutMemberParams,
  type ModerationUnbanServerMemberParams,
  type ModerationBanMemberParams,
...
import {
  ...
  type MessageDeleteReactionResponse,
...
import { Voice, type VoiceCreateTokenResponse } from './resources/voice';
...
export interface ClientOptions {
   ...
   */
  strafeBotToken?: string | AuthTokenProvider | undefined;

  /**
   * Override the default base URL for the API, e.g., "https://api.example.com/v2/"
   *
   * Defaults to process.env["DEFAULT_TEAM_JHY69_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["DEFAULT_TEAM_JHY69_LOG"] or 'warn' if it isn't set.
   */
...
export type StrafeBotAPIOptions = ClientOptions;

/**
 * API Client for interfacing with the StrafeBotApi API.
 */
export class StrafeBotAPI {
  strafeBotToken: string | AuthTokenProvider;
   ...
   * API Client for interfacing with the StrafeBotApi API.
   *
   * @param {string | AuthTokenProvider | undefined} [opts.strafeBotToken=process.env["STRAFE_BOT_TOKEN"] ?? undefined]
   * @param {string} [opts.baseURL=process.env["DEFAULT_TEAM_JHY69_BASE_URL"] ?? http://localhost:3000] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEFAULT_TEAM_JHY69_BASE_URL'),
    strafeBotToken = readEnv('STRAFE_BOT_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (strafeBotToken === undefined) {
      throw new Errors.StrafeBotAPIError(
        "The STRAFE_BOT_TOKEN environment variable is missing or empty; either provide it, or instantiate the StrafeBotAPI client with an strafeBotToken option, like new StrafeBotAPI({ strafeBotToken: 'My Strafe Bot Token' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? StrafeBotAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DEFAULT_TEAM_JHY69_LOG'), 'process.env["DEFAULT_TEAM_JHY69_LOG"]', this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DEFAULT_TEAM_JHY69_CUSTOM_HEADERS');
    if (customHeadersEnv) {
    ...
    this.strafeBotToken = strafeBotToken;
  }
  ...
  withOptions(options: Partial<ClientOptions>): this {
    const client = new (this.constructor as new (props: ClientOptions) => this)({
      ...
      strafeBotToken: this.strafeBotToken,
      ...options,
    });
    return client;
  }
  ...
  authHeadersSync(): Record<string, string> {
    ...
    const strafeBotToken = this.resolveAuthOptionSync('strafeBotToken', this.strafeBotToken);
    if (strafeBotToken) headers['Authorization'] = `Bearer ${strafeBotToken}`;
    return headers;
  }

  webSocketAuthHeaders(): Record<string, string> {
    const strafeBotToken = this.resolveAuthOptionSync('strafeBotToken', this.strafeBotToken);
    if (strafeBotToken) return { Authorization: `Bearer ${strafeBotToken}` };
    return {};
  }

  protected async authHeaders(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    const strafeBotToken = await this.resolveAuthOption('strafeBotToken', this.strafeBotToken);
    if (strafeBotToken == null) {
      return undefined;
    }
    return buildHeaders([{ Authorization: `Bearer ${strafeBotToken}` }]);
  }
  ...
  private async resolveAuthOption(
    ...
    if (!token)
      throw new Errors.StrafeBotAPIError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  static StrafeBotAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static StrafeBotAPIError = Errors.StrafeBotAPIError;
...
StrafeBotAPI.Users = Users;
StrafeBotAPI.Servers = Servers;
StrafeBotAPI.Channels = Channels;
StrafeBotAPI.Permissions = Permissions;
StrafeBotAPI.Roles = Roles;
StrafeBotAPI.Audit = Audit;
StrafeBotAPI.DirectMessages = DirectMessages;
StrafeBotAPI.Files = Files;
StrafeBotAPI.Members = Members;
StrafeBotAPI.Moderation = Moderation;
StrafeBotAPI.Messages = Messages;
StrafeBotAPI.Search = Search;
StrafeBotAPI.Voice = Voice;

export declare namespace StrafeBotAPI {
  ...
  export { Voice as Voice, type VoiceCreateTokenResponse as VoiceCreateTokenResponse };
}
